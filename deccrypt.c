...
int main(int argc, char *argv[]){
	...
	int fr = open("/media/external/encryptedFile.xor",O_RDONLY);
	int fw = open ("/media/external/decryptedFile.txt",O_WRONLY | O_CREAT);
	if (fr<0){
		printf("Error. Cannot open encryptedFile.xor.\n");
		exit(1);
	}
	if (fw<0){
		printf("Error. Cannot write decryptedFile.txt \n");
		exit(1);
	}
	...
	int f = write(fw,buffer_out,TAILLE_MAX);
